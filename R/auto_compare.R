...
compare_df <- function(prev_df, comp_df, scenarios = c("historical", "WM")) {
  ...
  difference_rows <-
    ...
    group_by(region, category, gas, unit) %>%
    ...
    group_by(region, category, unit, year) %>%
...
trends_df <- function(trend_df) {
  ...
  #   category, region, gas, year, unit, value
...
dataset_compare_list <- function() {
  ...
  return(c(emissions_datasets, ghgi_datasets))
}
